...
export default function overlayLine(selection,
  ...
  const topOverlayText = topOverlayLabelFormat(countAtPosition);
  const overlayDialogTopWidth = getTextWidth(topOverlayText) + 50;

  const bottomOverlayText = bottomOverlayLabelFormat(timeAtPosition);
  const overlayDialogBottomWidth = getTextWidth(bottomOverlayText) + 75;
  ...
  enteringGroup.select('.historical-counts-overlay-bottom-text').text(bottomOverlayText);

  enteringGroup.select('.historical-counts-overlay-top-text')
    .attr('transform', `translate(${renderPersonIcon ? overlayDialogTopTextCenterOffset : 0},0)`)
    .text(topOverlayText);
